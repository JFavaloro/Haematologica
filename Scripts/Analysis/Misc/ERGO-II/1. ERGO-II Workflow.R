...
# results from Walz et al (2015). (https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4608392/) who found # 197 peptides from 58 proteins to be overexpressed on MHC class-I in both myeloma cell lines and
# primary samples.
...
clonotypes = repLoad("Data/10xVDJ/", .mode = "single")

# 2.3 Load processed paired transcriptome data for PT43 & PT63 and set to appropriate resolution
...
# 3.1 Create patient clonotype files in the correct format for upload to ERGO-II
...
pt13pt31_trb = as.data.frame(pt13pt31_trb) # NB: rbindlist converts to data.table. Data needs to be in data.frame for mutate to function in section 3.2
...
# 3.3.1a Create the folder for PT13 & PT31 and point R to it
cidr = getwd()
mkfolder = "Data/ERGO/PT13_PT31/Upload/"
dir.create(file.path(cidr, mkfolder), recursive = T)

# 3.3.1b Write out the *.csv for PT13 & PT31
imap(ERGO_pt13pt31, function(df, name){
  write_csv(df, file = paste0(mkfolder, name, "_pt13pt31.csv"), na = "")
})

# 3.3.2a Create the folder for PT43 & PT63 and point R to it
cidr = getwd()
mkfolder = "Data/ERGO/PT43_PT63/Upload/"
dir.create(file.path(cidr, mkfolder), recursive = T)

# 3.3.2b Write out the *.csv for PT43 & PT63
...
cidr = getwd()
mkfolder = "Data/ERGO/PT13_PT31/Results/"
dir.create(file.path(cidr, mkfolder), recursive = T)
filenames <- list.files("Data/ERGO/PT13_PT31/Upload/") #Point to the directory where *.csv files are located
...
walk(filenames, function(x) {
  ...
  request = POST(target,
                 body = list(
                   ...
                   input_file = upload_file(paste0("Data/ERGO/PT13_PT31/Upload/", x)), #Point to the directory where *.csv files are located
...
cidr = getwd()
mkfolder = "Data/ERGO/PT43_PT63/Results/"
dir.create(file.path(cidr, mkfolder), recursive = T)
filenames <- list.files("Data/ERGO/PT43_PT63/Upload/") #Point to the directory where *.csv files are located
...
walk(filenames, function(x) {
  ...
  request = POST(target,
                 body = list(
                   # Emulate the manual form fields
                   input_file = upload_file(paste0("Data/ERGO/PT43_PT63/Upload/", x)), #Point to the directory where *.csv files are located
...
mydir = "Data/ERGO/PT13_PT31/Results/"
myfiles = list.files(path=mydir, pattern="*.csv", full.names=TRUE)
pt13pt31_results = map_dfr(myfiles,
                               read_csv)
...
mydir = "Data/ERGO/PT43_PT63/Results/"
myfiles = list.files(path=mydir, pattern="*.csv", full.names=TRUE)
pt43pt63_results = map_dfr(myfiles,
                           read_csv)
...
ERGO_results_wide = left_join(temp2, temp3, by = c("TRB" = "CDR3.aa", "TRBV" = "V.name", "TRBJ" = "J.name"))
...
# 6.2 Query clones with paired transcriptome data

# 6.2.1 Extract proportion of cells from our seurat object
seurat_clonotypes = table(x$CTaa, x$orig.ident) %>% as.data.frame.matrix() %>% rownames_to_column()
...
# 6.2.2 Merge these two into one sortable list - 
# NB: Need to fix this to keep one instance - none of these work because we've extracted this information from the seurat object where a clonotype can be an a/b or b etc. Not sure how to resolve this.
...
# 7. Write out results for further analysis --------------------------------------------------------

# Clean up the environment
rm(list=ls()[! ls() %in% c("clonotypes", "ERGO_results_long", "ERGO_results_long_seurat", "ERGO_results_wide", "ERGO_results_wide_seurat", "seurat_clonotypes", "x")])

# Save out the results for further analysis
save.image("Output/ERGO/ERGO_results.RData")

print("#># Finished running 'ERGO-II workflow' script")