# Find/Replace to go through the list

# ERGO_results_long
# ERGO_results_long_filtered
# ERGO_results_long_pb
# ERGO_results_long_pb_filtered
# ERGO_results_long_med_expansions
...
ggplot(peptide_results_tissue, aes(x=reorder(peptide, frequency), y=frequency, fill=tissue)) + 
  ...
  scale_fill_manual(values = plot_colours_rgb)
ggsave("Output/Figures/ERGO/peptide_results_tissue.png", width = 10, height = 5)

# 1.1b Barplot - Disease

# Prep the data into a format suitable for plotting
peptide_results_disease = table(ERGO_results_long$Peptide, ERGO_results_long$Disease) %>% as.data.frame()
...
ggplot(peptide_results_disease, aes(x=reorder(peptide, frequency), y=frequency, fill=disease)) + 
  ...
  theme(axis.text.x = element_text(angle = 90, size = 5)) +
  scale_y_continuous(expand = c(0,0)) +
  scale_fill_manual(values = plot_colours_brg)
...
venn.diagram(logi_results,
             ...
             col = plot_colours_rgb,
             fill = plot_colours_rgb,
             output=TRUE
)
...
# Prep the data into a format suitable for plotting
results = table(ERGO_results_long$Peptide, ERGO_results_long$Disease) %>% as.data.frame.array()
logi_results = results >= 1
logi_results = as.data.frame(logi_results) *1
# Adjust the logical results to a list to allow plotting
logi_results = lapply(logi_results, function(x) rownames(logi_results)[x==1])
...
venn.diagram(logi_results,
             ...
             col = plot_colours_brg,
             fill = plot_colours_brg,
             output=TRUE
)
...
# Prep the data into a format suitable for plotting
protein_results_disease = table(ERGO_results_long$Protein, ERGO_results_long$Disease) %>% as.data.frame()
...
# Prep the data into a format suitable for plotting
results = table(ERGO_results_long$Protein, ERGO_results_long$Disease) %>% as.data.frame.array()
logi_results = results >= 1
logi_results = as.data.frame(logi_results) *1
# Adjust the logical results to a list to allow plotting
logi_results = lapply(logi_results, function(x) rownames(logi_results)[x==1])
