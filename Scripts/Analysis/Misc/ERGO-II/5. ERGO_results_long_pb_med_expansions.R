...
peptide_results = table(ERGO_results_long_pb_med_expansions$Peptide) %>% as.data.frame()
...
peptide_results_tissue = table(ERGO_results_long_pb_med_expansions$Peptide, ERGO_results_long_pb_med_expansions$Tissue) %>% as.data.frame()
...
# Prep the data into a format suitable for plotting
peptide_results_disease = table(ERGO_results_long_pb_med_expansions$Peptide, ERGO_results_long_pb_med_expansions$Disease) %>% as.data.frame()
...
# Prep the data into a format suitable for plotting
results = table(ERGO_results_long_pb_med_expansions$Peptide, ERGO_results_long_pb_med_expansions$Tissue) %>% as.data.frame.array()
...
# Prep the data into a format suitable for plotting
results = table(ERGO_results_long_pb_med_expansions$Peptide, ERGO_results_long_pb_med_expansions$Disease) %>% as.data.frame.array()
logi_results = results >= 1
logi_results = as.data.frame(logi_results) *1
# Adjust the logical results to a list to allow plotting
logi_results = lapply(logi_results, function(x) rownames(logi_results)[x==1])
...
# Prep the data into a format suitable for plotting
results = table(ERGO_results_long_pb_med_expansions$Peptide, ERGO_results_long_pb_med_expansions$Sample) %>% as.data.frame.array()
logi_results = results >= 1
logi_results = as.data.frame(logi_results) *1
# Adjust the logical results to a list to allow plotting
logi_results = lapply(logi_results, function(x) rownames(logi_results)[x==1])
...
top_10_peptides = subset(ERGO_results_long_pb_med_expansions, subset = ERGO_results_long_pb_med_expansions$Peptide %in% top_10_peptides)
...
# Set our global analysis to allow plotting later
protein_results = table(ERGO_results_long_pb_med_expansions$Protein) %>% as.data.frame()
...
# Prep the data into a format suitable for plotting
protein_results_tissue = table(ERGO_results_long_pb_med_expansions$Protein, ERGO_results_long_pb_med_expansions$Tissue) %>% as.data.frame()
...
# Prep the data into a format suitable for plotting
protein_results_disease = table(ERGO_results_long_pb_med_expansions$Protein, ERGO_results_long_pb_med_expansions$Disease) %>% as.data.frame()
...
# Prep the data into a format suitable for plotting
results = table(ERGO_results_long_pb_med_expansions$Protein, ERGO_results_long_pb_med_expansions$Tissue) %>% as.data.frame.array()
logi_results = results >= 1
logi_results = as.data.frame(logi_results) *1
# Adjust the logical results to a list to allow plotting
logi_results = lapply(logi_results, function(x) rownames(logi_results)[x==1])
...
# Prep the data into a format suitable for plotting
results = table(ERGO_results_long_pb_med_expansions$Protein, ERGO_results_long_pb_med_expansions$Disease) %>% as.data.frame.array()
logi_results = results >= 1
logi_results = as.data.frame(logi_results) *1
# Adjust the logical results to a list to allow plotting
logi_results = lapply(logi_results, function(x) rownames(logi_results)[x==1])
...
# Prep the data into a format suitable for plotting
results = table(ERGO_results_long_pb_med_expansions$Protein, ERGO_results_long_pb_med_expansions$Sample) %>% as.data.frame.array()
logi_results = results >= 1
logi_results = as.data.frame(logi_results) *1
# Adjust the logical results to a list to allow plotting
logi_results = lapply(logi_results, function(x) rownames(logi_results)[x==1])
...
top_10_proteins = subset(ERGO_results_long_pb_med_expansions, subset = ERGO_results_long_pb_med_expansions$Protein %in% top_10_proteins)
