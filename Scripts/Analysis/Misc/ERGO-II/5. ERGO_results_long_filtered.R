...
table(ERGO_results_wide_filtered$Sample)

# Tabulate the number of hits against hyperexpanded clones (i.e. >10% of total repertoire)
table(ERGO_results_wide_filtered$Proportion >= 0.1, ERGO_results_wide_filtered$Sample)

# Tabulate the number of hits against large clones (i.e. between 1-10% of total repertoire)
table(ERGO_results_wide_filtered$Proportion >= 0.01 & ERGO_results_wide_filtered$Proportion < 0.1, ERGO_results_wide_filtered$Sample)

# Tabulate the number of hits against medium clones (i.e. between 0.1-1% of total repertoire)
table(ERGO_results_wide_filtered$Proportion >= 0.001 & ERGO_results_wide_filtered$Proportion < 0.01, ERGO_results_wide_filtered$Sample)

# Tabulate the number of hits against small clones (i.e. between 0.01-0.01% of total repertoire)
table(ERGO_results_wide_filtered$Proportion >= 0.0001 & ERGO_results_wide_filtered$Proportion < 0.001, ERGO_results_wide_filtered$Sample)

# Tabulate the number of hits against single clones (i.e. single reads)
table(ERGO_results_wide_filtered$Clones == 1, ERGO_results_wide_filtered$Sample)
...
peptide_results = table(ERGO_results_long_filtered$Peptide) %>% as.data.frame()
...
peptide_results_tissue = table(ERGO_results_long_filtered$Peptide, ERGO_results_long_filtered$Tissue) %>% as.data.frame()
...
# Prep the data into a format suitable for plotting
peptide_results_disease = table(ERGO_results_long_filtered$Peptide, ERGO_results_long_filtered$Disease) %>% as.data.frame()
...
# Prep the data into a format suitable for plotting
results = table(ERGO_results_long_filtered$Peptide, ERGO_results_long_filtered$Tissue) %>% as.data.frame.array()
...
# Prep the data into a format suitable for plotting
results = table(ERGO_results_long_filtered$Peptide, ERGO_results_long_filtered$Disease) %>% as.data.frame.array()
logi_results = results >= 1
logi_results = as.data.frame(logi_results) *1
# Adjust the logical results to a list to allow plotting
logi_results = lapply(logi_results, function(x) rownames(logi_results)[x==1])
...
# Prep the data into a format suitable for plotting
results = table(ERGO_results_long_filtered$Peptide, ERGO_results_long_filtered$Sample) %>% as.data.frame.array()
logi_results = results >= 1
logi_results = as.data.frame(logi_results) *1
# Adjust the logical results to a list to allow plotting
logi_results = lapply(logi_results, function(x) rownames(logi_results)[x==1])
...
top_10_peptides = subset(ERGO_results_long_filtered, subset = ERGO_results_long_filtered$Peptide %in% top_10_peptides)
...
# Set our global analysis to allow plotting later
protein_results = table(ERGO_results_long_filtered$Protein) %>% as.data.frame()
...
# Prep the data into a format suitable for plotting
protein_results_tissue = table(ERGO_results_long_filtered$Protein, ERGO_results_long_filtered$Tissue) %>% as.data.frame()
...
# Prep the data into a format suitable for plotting
protein_results_disease = table(ERGO_results_long_filtered$Protein, ERGO_results_long_filtered$Disease) %>% as.data.frame()
...
# Prep the data into a format suitable for plotting
results = table(ERGO_results_long_filtered$Protein, ERGO_results_long_filtered$Tissue) %>% as.data.frame.array()
logi_results = results >= 1
logi_results = as.data.frame(logi_results) *1
# Adjust the logical results to a list to allow plotting
logi_results = lapply(logi_results, function(x) rownames(logi_results)[x==1])
...
# Prep the data into a format suitable for plotting
results = table(ERGO_results_long_filtered$Protein, ERGO_results_long_filtered$Disease) %>% as.data.frame.array()
logi_results = results >= 1
logi_results = as.data.frame(logi_results) *1
# Adjust the logical results to a list to allow plotting
logi_results = lapply(logi_results, function(x) rownames(logi_results)[x==1])
...
# Prep the data into a format suitable for plotting
results = table(ERGO_results_long_filtered$Protein, ERGO_results_long_filtered$Sample) %>% as.data.frame.array()
logi_results = results >= 1
logi_results = as.data.frame(logi_results) *1
# Adjust the logical results to a list to allow plotting
logi_results = lapply(logi_results, function(x) rownames(logi_results)[x==1])
...
top_10_proteins = subset(ERGO_results_long_filtered, subset = ERGO_results_long_filtered$Protein %in% top_10_proteins)
